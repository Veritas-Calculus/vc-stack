...
func (s *Service) SetupRoutes(router *gin.Engine) {
	g := router.Group("/api/v1/encryption")
	{
		...
		g.GET("/profiles", s.listProfiles)
		g.POST("/profiles", s.createProfile)
		g.GET("/profiles/:id", s.getProfile)
		g.PUT("/profiles/:id", s.updateProfile)
		g.DELETE("/profiles/:id", s.deleteProfile)

		// Volume encryption management.
		g.GET("/volumes", s.listEncryptedVolumes)
		g.POST("/volumes/:volume_id/encrypt", s.encryptVolume)
		g.GET("/volumes/:volume_id", s.getVolumeEncryption)
		g.DELETE("/volumes/:volume_id", s.removeVolumeEncryption)

		// mTLS certificate management.
		g.GET("/mtls/certificates", s.listCertificates)
		g.POST("/mtls/certificates", s.issueCertificate)
		g.GET("/mtls/certificates/:id", s.getCertificate)
		g.POST("/mtls/certificates/:id/revoke", s.revokeCertificate)
		g.GET("/mtls/ca", s.getCACert)

		// Compliance overview.
		g.GET("/compliance", s.getCompliance)
	}
}
