...
func (m *RBDManager) CreateVolume(pool, name string, sizeGB int) error {
	...
	cmd := exec.Command("rbd", m.rbdArgs("volumes", "create", fmt.Sprintf("%s/%s", pool, name), "--size", sizeArg)...) // #nosec
...
func (m *RBDManager) DeleteVolume(pool, name string) error {
	cmd := exec.Command("rbd", m.rbdArgs("volumes", "rm", fmt.Sprintf("%s/%s", pool, name))...) // #nosec
	if out, err := cmd.CombinedOutput(); err != nil {
...
func (m *RBDManager) ResizeVolume(pool, name string, newSizeGB int) error {
	...
	cmd := exec.Command("rbd", m.rbdArgs("volumes", "resize", fmt.Sprintf("%s/%s", pool, name), "--size", sizeArg)...) // #nosec
	if out, err := cmd.CombinedOutput(); err != nil {
