...
interface DRDrill { id: string; plan_id: string; name: string; type: string; status: string; started_at: string; completed_at: string; rpo_achieved_minutes: number; rto_achieved_minutes: number; rpo_met: boolean; rto_met: boolean; recovered_vms: number; total_vms: number; notes: string }
interface FailoverEvt { id: string; plan_id: string; type: string; status: string; reason: string; started_at: string; completed_at: string; duration_seconds: number; affected_vms: number; notes: string }
...
export function DisasterRecovery() {
    ...
    const fetchAll = useCallback(async () => {
        try {
            const [st, si, pl] = await Promise.allSettled([
                api.get('/v1/dr/status'), api.get('/v1/dr/sites'), api.get('/v1/dr/plans')
            ])
    ...
    const fetchDrills = useCallback(async () => {
        try { const r = await api.get('/v1/dr/drills'); setDrills(r.data.drills || []) } catch { /* */ }
    }, [])

    const fetchEvents = useCallback(async () => {
        try { const r = await api.get('/v1/dr/failover-events'); setEvents(r.data.events || []) } catch { /* */ }
    }, [])

    useEffect(() => { fetchAll() }, [fetchAll])
    useEffect(() => { if (tab === 'drills') fetchDrills() }, [tab, fetchDrills])
    useEffect(() => { if (tab === 'events') fetchEvents() }, [tab, fetchEvents])

    const runDrill = async () => {
        if (!plans.length) return
        try {
            await api.post('/v1/dr/drills', { plan_id: plans[0].id, name: `DR Drill ${new Date().toISOString().slice(0, 10)}`, type: 'planned' })
            fetchDrills()
        } catch { /* */ }
    }
    ...
    const doFailover = async () => {
        if (!plans.length) return
        try { await api.post('/v1/dr/failover', { plan_id: plans[0].id, reason: 'Manual failover initiated' }); fetchAll(); fetchEvents() } catch { /* */ }
    }

    const doFailback = async () => {
        if (!plans.length) return
        try { await api.post('/v1/dr/failback', { plan_id: plans[0].id }); fetchAll(); fetchEvents() } catch { /* */ }
    }

    const badge = (s: string) => {
        const m: Record<string, string> = {
            active: 'bg-emerald-500/20 text-emerald-400', primary: 'bg-blue-500/20 text-blue-400',
            warm_standby: 'bg-amber-500/20 text-amber-400', cold_standby: 'bg-gray-500/20 text-gray-400',
            offline: 'bg-red-500/20 text-red-400', failover_active: 'bg-purple-500/20 text-purple-400',
            degraded: 'bg-orange-500/20 text-orange-400', critical: 'bg-red-500/20 text-red-400',
            high: 'bg-orange-500/20 text-orange-400', medium: 'bg-amber-500/20 text-amber-400',
            low: 'bg-gray-500/20 text-gray-400', completed: 'bg-emerald-500/20 text-emerald-400',
            failed: 'bg-red-500/20 text-red-400', running: 'bg-blue-500/20 text-blue-400',
            sync: 'bg-blue-500/20 text-blue-400', async: 'bg-cyan-500/20 text-cyan-400',
            scheduled: 'bg-purple-500/20 text-purple-400',
            failover: 'bg-red-500/20 text-red-400', failback: 'bg-emerald-500/20 text-emerald-400',
    ...
    const tabs: { key: Tab; label: string }[] = [
        ...
        { key: 'plans', label: 'DR Plans' }, { key: 'drills', label: 'Drills' }, { key: 'events', label: 'Failover Events' },
    ]
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-6">
                            ...
                            {sites.map(site => (
                                <div key={site.id} className={`bg-gray-700/30 border rounded-xl p-5 min-w-[200px] text-center ${site.status === 'active' ? 'border-emerald-500/40' : site.status === 'failover_active' ? 'border-purple-500/40' : 'border-red-500/40'}`}>
                                    ...
                                    <div className="mt-3 text-xs text-gray-400">
                                        <div>Storage: {site.storage_used_gb}/{site.storage_total_gb} GB</div>
                                        <div className="mt-1"><div className="w-full h-1.5 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${site.storage_total_gb ? (site.storage_used_gb / site.storage_total_gb * 100) : 0}%` }}></div></div></div>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {plans.length > 0 && (
                            <div className="flex items-center justify-center gap-2 mt-4 text-gray-400">
                                ...
                                <span className="text-cyan-400">→→→ {plans[0].replication_type} replication →→→</span>
                                <span className="text-sm">{siteForId(plans[0].target_site_id)}</span>
                                <span className="text-xs text-gray-500 ml-4">lag: {plans[0].replication_lag_seconds}s</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                        <div className="px-5 py-3 border-b border-gray-700/30">
                            <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">DR Plans</h3>
                        </div>
                        <table className="w-full text-sm">
                            <thead className="bg-gray-700/30"><tr className="text-left text-gray-400 text-xs uppercase">
                                <th className="px-4 py-3">Plan</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">RPO</th><th className="px-4 py-3">RTO</th><th className="px-4 py-3">Replication</th><th className="px-4 py-3">Protected</th><th className="px-4 py-3">Lag</th><th className="px-4 py-3">Status</th>
                            </tr></thead>
                            <tbody>{plans.map(p => (
                                <tr key={p.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition">
                                    <td className="px-4 py-3"><div className="text-white font-medium">{p.name}</div><div className="text-gray-500 text-xs">{p.description}</div></td>
                                    ...
                                    <td className="px-4 py-3"><span className={badge(p.replication_type)}>{p.replication_type}</span></td>
                                    <td className="px-4 py-3 text-white">{p.protected_count} resources</td>
                                    <td className="px-4 py-3 text-gray-300 font-mono text-xs">{p.replication_lag_seconds}s</td>
            ...
            {tab === 'sites' && (
                <div className="grid gap-4">
                    {sites.map(site => (
                        <div key={site.id} className={`bg-gray-800/60 border rounded-xl p-5 ${site.healthy ? 'border-gray-700/40' : 'border-red-500/40'}`}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-gray-400 text-sm">{site.location} • {site.endpoint}</div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right text-xs">
                                        <div className="text-gray-400">Storage</div>
                                        <div className="text-white font-bold">{site.storage_used_gb} / {site.storage_total_gb} GB</div>
                                        <div className="mt-1 w-32"><div className="w-full h-1.5 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${site.storage_total_gb ? (site.storage_used_gb / site.storage_total_gb * 100) : 0}%` }}></div></div></div>
                                    </div>
                        ...
                        <div key={p.id} className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                            ...
                            <div className="grid grid-cols-5 gap-4 mb-3">
                                ...
                                <div className="bg-gray-700/30 rounded-lg p-3 text-center">
                                    ...
                                    <div className={`font-bold text-lg ${p.replication_lag_seconds < 30 ? 'text-emerald-400' : p.replication_lag_seconds < 60 ? 'text-amber-400' : 'text-red-400'}`}>{p.replication_lag_seconds}s</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4 text-xs text-gray-400 mt-2">
                                ...
                                {p.last_replication && <span className="ml-auto">Last sync: {new Date(p.last_replication).toLocaleString()}</span>}
                            </div>
                        </div>
                    ))}
                </div>
                                ...
                                <div key={d.id} className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                                    ...
                                    <div className="grid grid-cols-4 gap-4 mb-3">
                                        ...
                                        <div className="bg-gray-700/30 rounded-lg p-3 text-center">
                                            ...
                                            <div className="text-white font-bold">{d.recovered_vms}/{d.total_vms}</div>
                                        </div>
                                        <div className="bg-gray-700/30 rounded-lg p-3 text-center">
                                            <div className="text-gray-500 text-xs mb-1">Started</div>
                                            <div className="text-gray-300 text-xs">{d.started_at ? new Date(d.started_at).toLocaleString() : '—'}</div>
                                        </div>
                                    </div>
                ...
                <div className="space-y-4">
                    {events.length === 0 ? (
                        ...
                        <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                            <table className="w-full text-sm">
                                <thead className="bg-gray-700/30"><tr className="text-left text-gray-400 text-xs uppercase">
                                    <th className="px-4 py-3">Type</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Reason</th><th className="px-4 py-3">Duration</th><th className="px-4 py-3">Affected VMs</th><th className="px-4 py-3">Time</th>
                                </tr></thead>
                                <tbody>{events.map(e => (
                                    <tr key={e.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition">
                                        <td className="px-4 py-3"><span className={badge(e.type)}>{e.type}</span></td>
                                        <td className="px-4 py-3"><span className={badge(e.status)}>{e.status}</span></td>
                                        <td className="px-4 py-3 text-gray-300">{e.reason}</td>
                                        <td className="px-4 py-3 text-white font-mono">{Math.round(e.duration_seconds / 60)}m</td>
                                        <td className="px-4 py-3 text-white">{e.affected_vms}</td>
                                        <td className="px-4 py-3 text-gray-400 text-xs">{new Date(e.started_at).toLocaleString()}</td>
                                    </tr>
                                ))}</tbody>
                            </table>
                        </div>
