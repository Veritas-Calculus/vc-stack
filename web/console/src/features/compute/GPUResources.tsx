...
import { fetchPhysicalGPUs, fetchGPUProfiles, type UIPhysicalGPU, type UIGPUProfile } from '@/lib/api'
...
export function GPUResources() {
    ...
    const profCols: Column<UIGPUProfile>[] = [
        { key: 'name', header: 'Profile', render: (r) => <code className="text-sm font-medium">{r.name}</code> },
        { key: 'vram_mb', header: 'VRAM', render: (r) => <span className="text-sm">{vramFmt(r.vram_mb)}</span> },
        { key: 'compute', header: 'Compute', render: (r) => <div className="flex items-center gap-2"><div className="w-24 h-2 bg-zinc-800 rounded-full overflow-hidden"><div className="h-full bg-purple-500 rounded-full" style={{ width: `${r.compute}%` }} /></div><span className="text-xs text-zinc-400">{r.compute}%</span></div> },
        { key: 'max_per_gpu', header: 'Max/GPU', render: (r) => <span className="text-sm">{r.max_per_gpu}</span> },
        { key: 'description', header: 'Description', render: (r) => <span className="text-xs text-zinc-400">{r.description}</span> },
    ]

    return (
        <div className="space-y-6">
            <PageHeader title="GPU Resources" subtitle="Physical GPU inventory and vGPU partitioning profiles" />
            {loading ? <div className="text-center py-12 text-zinc-500">Loading...</div> : (
                <>
                    <div><h3 className="text-sm font-medium text-zinc-300 mb-2">Physical GPUs</h3><DataTable columns={cols} data={gpus} empty="No GPUs registered" /></div>
                    <div><h3 className="text-sm font-medium text-zinc-300 mb-2">vGPU Profiles (MIG)</h3><DataTable columns={profCols} data={profiles} empty="No profiles" /></div>
                </>
            )}
        </div>
    )
