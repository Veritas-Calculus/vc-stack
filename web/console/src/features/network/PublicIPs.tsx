...
export function PublicIPs() {
  ...
  const [rows, setRows] = useState<
    Array<{ id: string; address: string; status: string; port_id?: string; fixed_ip?: string }>
  >([])
  ...
  const attachedLabel = useMemo(
    () => (row: { port_id?: string; fixed_ip?: string }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <Modal
        ...
        footer={
          <>
            <button className="btn-secondary" onClick={() => setAssocOpen(false)}>
              Cancel
            </button>
            <button
              className="btn-primary"
              ...
              onClick={async () => {
                ...
                const f = await updateFloatingIP(assocFipId, {
              ...
              {ports
                ...
                .map((p) => (
                  <option key={p.id} value={p.id}>
                    ...
                    {p.fixed_ips && p.fixed_ips[0]?.ip ? p.fixed_ips[0].ip : p.id}
                  </option>
                ))}
            </select>
          </div>
