...
export default function PortManagement() {
    ...
    const columns: Column<Record<string, unknown>>[] = [
        { key: 'name', header: 'Name', render: (r) => <span>{(r as unknown as Port).name || (r as unknown as Port).id.toString().slice(0, 8)}</span> },
        {
            key: 'fixed_ips', header: 'IP Address', render: (r) => {
                const port = r as unknown as Port
                return (<div className="space-y-0.5">
                    {(port.fixed_ips || []).map((f, i) => (
                        <code key={i} className="text-xs block">{f.ip}</code>
                    ))}
                </div>)
            }
        },
        { key: 'mac_address', header: 'MAC', render: (r) => <code className="text-xs">{(r as unknown as Port).mac_address}</code> },
        {
            key: 'network_id', header: 'Network', render: (r) => {
                const port = r as unknown as Port
                return <span>{port.network?.name || port.network_id.slice(0, 8)}</span>
            }
        },
        {
            key: 'device_owner', header: 'Device', render: (r) => {
                const port = r as unknown as Port
                ...
                return (
                    <div>
                        ...
                        {port.device_id && <div className="text-xs text-neutral-400 mt-0.5">{port.device_id.slice(0, 8)}</div>}
                    </div>
                )
            }
        },
        {
            key: 'status', header: 'Status', render: (r) => {
                const port = r as unknown as Port
                const variant = port.status === 'active' ? 'success' : port.status === 'building' ? 'warning' : 'default'
