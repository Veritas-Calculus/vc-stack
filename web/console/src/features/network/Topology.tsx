...
function layoutNodes(nodes: TopoNode[], _edges: TopoEdge[], width: number, height: number): NodePositionMap {
...
export default function NetworkTopology() {
    ...
    const load = useCallback(async () => {
        setLoading(true)
        try {
            const res = await api.get<{ nodes: TopoNode[]; edges: TopoEdge[]; stats: TopoStats }>('/v1/networks/topology')
                ...
                <div className="card p-4 space-y-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <span className="inline-block w-3 h-3 rounded-full" style={{ backgroundColor: NODE_COLORS[selected.type] }} />
                            <h3 className="text-sm font-medium">{selected.label}</h3>
                            <Badge variant="default">{selected.type}</Badge>
                        </div>
                        <button className="text-xs text-neutral-400 hover:text-white" onClick={() => setSelected(null)}>
                            Close
                        </button>
                    </div>
                    <div className="text-xs text-neutral-400">ID: <code>{selected.id}</code></div>
                    {selected.data && (
