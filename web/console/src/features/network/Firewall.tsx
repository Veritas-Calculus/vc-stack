...
export default function FirewallManagement() {
    ...
    const handleCreate = async () => {
        if (!newName) return
        try {
            await api.post('/v1/firewall-policies', { name: newName, description: newDesc, tenant_id: 'default' })
            setShowCreate(false)
            setNewName('')
            setNewDesc('')
            load()
        ...
        { key: 'shared', header: 'Shared', render: (r) => <span>{r.shared ? 'Yes' : 'No'}</span> },
        {
            key: 'actions', header: '', render: (r) => (
                <div className="flex gap-2">
                    <button className="btn-secondary text-xs" onClick={() => { setExpanded(expanded === r.id ? null : r.id); setShowAddRule(false) }}>
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <TableToolbar>
                ...
                <button className="btn-primary" onClick={() => setShowCreate(true)}>Create Policy</button>
            </TableToolbar>
            ...
            {showCreate && (
                <div className="card p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <input className="form-input" placeholder="Policy Name" value={newName} onChange={e => setNewName(e.target.value)} />
            ...
            <DataTable columns={columns as unknown as Column<Record<string, unknown>>[]} data={policies as unknown as Record<string, unknown>[]} empty="No firewall policies" />
                    ...
                    {showAddRule && (
                        <div className="card p-3 space-y-3 bg-neutral-800/50">
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                <input className="form-input" placeholder="Rule Name" value={ruleName} onChange={e => setRuleName(e.target.value)} />
                                <select className="form-input" value={ruleProtocol} onChange={e => setRuleProtocol(e.target.value)}>
                                    <option value="any">Any</option>
                                    <option value="tcp">TCP</option>
                                    <option value="udp">UDP</option>
                                    <option value="icmp">ICMP</option>
                                </select>
                                <select className="form-input" value={ruleAction} onChange={e => setRuleAction(e.target.value)}>
                                    <option value="allow">Allow</option>
                                    <option value="deny">Deny</option>
                                    <option value="reject">Reject</option>
                                </select>
                                <select className="form-input" value={ruleDirection} onChange={e => setRuleDirection(e.target.value)}>
                                    <option value="ingress">Ingress</option>
                                    <option value="egress">Egress</option>
                                </select>
                            </div>
                            <div className="grid grid-cols-4 gap-2">
                                <input className="form-input" placeholder="Source CIDR" value={ruleSrcIP} onChange={e => setRuleSrcIP(e.target.value)} />
                                <input className="form-input" placeholder="Destination CIDR" value={ruleDstIP} onChange={e => setRuleDstIP(e.target.value)} />
                                <input className="form-input" placeholder="Dest Port" type="number" value={ruleDstPort} onChange={e => setRuleDstPort(e.target.value)} />
                                <input className="form-input" placeholder="Position" type="number" value={rulePosition} onChange={e => setRulePosition(e.target.value)} />
                            </div>
                            <div className="flex gap-2">
                                <button className="btn-primary text-xs" onClick={handleAddRule}>Add</button>
                                <button className="btn-secondary text-xs" onClick={() => setShowAddRule(false)}>Cancel</button>
                            </div>
                        </div>
                    )}
                        ...
                        <tbody>
                            {(policies.find(p => p.id === expanded)?.rules || []).map((rule, idx) => (
                                <tr key={rule.id} className="border-b border-neutral-800 hover:bg-neutral-800/30">
                                    ...
                                    <td className="py-2 px-2"><code className="text-xs">{rule.protocol}</code></td>
                                    ...
                                    <td className="py-2 px-2"><code className="text-xs">{rule.source_ip || 'any'}</code></td>
                                    <td className="py-2 px-2"><code className="text-xs">{rule.destination_ip || 'any'}</code></td>
                                    <td className="py-2 px-2">{rule.destination_port || '—'}</td>
                                    <td className="py-2 px-2">{rule.enabled ? '✓' : '✗'}</td>
                                    <td className="py-2 px-2">
                                        <button className="text-red-400 text-xs hover:underline" onClick={() => handleDeleteRule(expanded, rule.id)}>
                                            Delete
                                        </button>
                                    </td>
                                </tr>
                            ...
                            {(!policies.find(p => p.id === expanded)?.rules?.length) && (
                                <tr><td colSpan={10} className="py-4 text-center text-neutral-500">No rules</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
