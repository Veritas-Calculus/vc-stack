...
export default function QoSManagement() {
    ...
    const columns: Column<UIQoSPolicy>[] = [
        { key: 'name', header: 'Name' },
        {
            ...
            header: 'Direction',
            render: (r) => (
                <span className={`text-xs px-1.5 py-0.5 rounded ${r.direction === 'egress' ? 'bg-blue-900/30 text-blue-400' : 'bg-purple-900/30 text-purple-400'
                    }`}>
        ...
        },
        {
            ...
            render: (r) => (
                <div>
                    ...
                    {r.max_burst_kb > 0 && (
                        <span className="text-xs text-gray-500 ml-1">(burst: {formatBandwidth(r.max_burst_kb)})</span>
                    )}
                </div>
            )
        },
        {
            ...
            render: (r) => (
                <span className="text-xs text-gray-300">
                    {r.network_id ? `Network: ${getNetworkName(r.network_id)}` : r.port_id ? `Port: ${r.port_id.slice(0, 8)}...` : 'Global'}
                </span>
            )
        },
        {
    ...
    return (
        <div className="space-y-3">
            ...
            <DataTable columns={columns} data={filtered} empty={loading ? 'Loading...' : 'No QoS policies'} />
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            />
                            {maxKbps && (
                                <p className="text-xs text-emerald-400 mt-1">{formatBandwidth(parseInt(maxKbps) || 0)}</p>
                            )}
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <select className="input w-full" value={networkId} onChange={(e) => setNetworkId(e.target.value)}>
            ...
            <Modal
                ...
                footer={
                    <>
                        ...
                        <button className="btn-primary" onClick={handleEditSave} disabled={loading || !editKbps}>
                            {loading ? 'Saving...' : 'Save'}
                        </button>
                    </>
                }
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            />
                            {editKbps && (
                                <p className="text-xs text-emerald-400 mt-1">{formatBandwidth(parseInt(editKbps) || 0)}</p>
                            )}
                        </div>
                        <div>
                            <label className="label">Burst (Kb)</label>
