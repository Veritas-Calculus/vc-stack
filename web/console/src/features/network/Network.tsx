...
function VPCPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [nets, zs] = await Promise.all([fetchNetworks(projectId), fetchZones()])
      setRows(nets)
      setZones(zs)
    } finally {
      setLoading(false)
...
function SGPage() {
  ...
  const cols: Column<Rule>[] = [
    ...
    { key: 'cidr', header: 'CIDR' },
    {
      ...
      render: () => (
        <div className="flex justify-end">
          <ActionMenu actions={[{ label: 'Delete', onClick: () => {}, danger: true }]} />
        </div>
      )
    }
  ]
  return (
    <div className="space-y-3">
      <PageHeader
        ...
        actions={<button className="btn-primary">Add Rule</button>}
      />
