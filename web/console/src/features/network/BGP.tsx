...
const stateVariant = (s: string): 'success' | 'warning' | 'danger' | 'default' => {
  ...
  if (s === 'active' || s === 'connect' || s === 'open_sent' || s === 'open_confirm')
    return 'warning'
...
export default function BGPManagement() {
  ...
  const peerCols: Column<BGPPeer>[] = [
    ...
    { key: 'local_asn', header: 'Local ASN' },
    {
      key: 'state',
      header: 'State',
      render: (r) => <Badge variant={stateVariant(r.state)}>{r.state}</Badge>
    },
    {
      key: 'auth_type',
      header: 'Auth',
  ...
  const rangeCols: Column<ASNRange>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <span>
          {r.allocated_count} / {r.total_count}
        </span>
      )
    }
  ]
  ...
  const allocCols: Column<ASNAllocation>[] = [
    { key: 'asn', header: 'ASN' },
    {
      ...
      render: (r) => <Badge variant="default">{r.resource_type}</Badge>
    },
    {
      key: 'resource_id',
      header: 'Resource ID',
      render: (r) => <span className="font-mono text-xs">{r.resource_id}</span>
    },
    { key: 'zone_id', header: 'Zone' }
  ]
  ...
  const routeCols: Column<AdvertisedRoute>[] = [
    ...
    { key: 'local_pref', header: 'LOCAL_PREF' },
    {
      key: 'source_type',
      header: 'Source',
      render: (r) => <Badge variant="default">{r.source_type}</Badge>
    },
    {
      key: 'status',
      header: 'Status',
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'allocations' && <DataTable columns={allocCols} data={allocs} />}

      {tab === 'routes' && <DataTable columns={routeCols} data={routes} />}
      ...
      <Modal open={showPeerModal} onClose={() => setShowPeerModal(false)} title="Add BGP Peer">
        <form onSubmit={handleCreatePeer} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="bfd" className="text-sm">
              Enable BFD (fast failure detection)
            </label>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2">
            <button type="button" className="btn-secondary" onClick={() => setShowPeerModal(false)}>
              Cancel
            </button>
