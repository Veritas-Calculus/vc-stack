...
export default function PortForwarding() {
    ...
    const filtered = useMemo(() => {
        ...
        return rules.filter((r) =>
            [r.internal_ip, r.protocol, r.description, String(r.external_port), String(r.internal_port)]
    ...
    const handleDelete = async (pf: UIPortForwarding) => {
        if (!confirm(`Delete port forwarding ${getFipAddress(pf.floating_ip_id)}:${pf.external_port}?`)) return
        setLoading(true)
        try {
    ...
    const columns: Column<UIPortForwarding>[] = [
        ...
        },
        {
            ...
            render: (r) => (
                <span className="font-mono">{r.external_port}</span>
            )
        },
        {
            key: 'direction',
        ...
        },
        {
            ...
            render: (r) => (
                <span className="font-mono text-emerald-400">{r.internal_ip}:{r.internal_port}</span>
            )
        },
        {
            key: 'protocol',
        ...
        },
        {
            ...
            render: (pf) => (
                <div className="flex justify-end">
                    <ActionMenu
                        actions={[
                            { label: 'Delete', onClick: () => handleDelete(pf), danger: true }
                        ]}
                    />
                </div>
            )
    ...
    return (
        <div className="space-y-3">
            ...
            <DataTable columns={columns} data={filtered} empty={loading ? 'Loading...' : 'No port forwarding rules'} />
            ...
            >
                <div className="space-y-4">
                    <div>
                        ...
                        <select className="input w-full" value={fipId} onChange={(e) => setFipId(e.target.value)}>
                    ...
                    <div className="p-3 bg-blue-900/20 border border-blue-800/30 rounded text-sm text-gray-300">
                        Traffic to <span className="font-mono text-blue-400">{fipId ? getFipAddress(fipId) : '<FIP>'}:{extPort || '<port>'}</span>{' '}
                        will be forwarded to <span className="font-mono text-emerald-400">{intIp || '<IP>'}:{intPort || '<port>'}</span> via OVN DNAT.
                    </div>
                </div>
            </Modal>
        </div>
