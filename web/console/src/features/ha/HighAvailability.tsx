...
export function HighAvailability() {
    const [tab, setTab] = useState<'overview' | 'policies' | 'instances' | 'evacuations' | 'fencing'>('overview')
    ...
    const [selectedEvac, setSelectedEvac] = useState<{ event: EvacuationEvent; instances: EvacuationInstance[] } | null>(null)
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (fencingRes.status === 'fulfilled') setFencingEvents(fencingRes.value.data.fencing_events || [])
    ...
    const toggleInstanceHA = async (instanceId: number, enable: boolean) => {
        try {
            ...
            fetchData()
        } catch (err) { console.error('Toggle HA error:', err) }
    }
    ...
    const viewEvacDetails = async (evac: EvacuationEvent) => {
        ...
        } catch (err) { console.error('Fetch evac details error:', err) }
    }
    ...
    const unfenceHost = async (hostId: string) => {
        try {
            ...
            fetchData()
        } catch (err) { console.error('Unfence error:', err) }
    }
    ...
    const tabs = [
        ...
        { key: 'instances' as const, label: 'Protected Instances', count: instances.filter(i => i.ha_enabled).length },
        { key: 'evacuations' as const, label: 'Evacuations', count: evacuations.length },
        { key: 'fencing' as const, label: 'Fencing', count: fencingEvents.filter(f => f.status === 'fenced').length },
    ]
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Node fencing, VM evacuation, and HA policy management</p>
                </div>
                {status && (
                    <div className="flex items-center gap-3">
                        <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium ${status.ha_enabled ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                            <span className={`w-2 h-2 rounded-full mr-2 ${status.ha_enabled ? 'bg-emerald-400 animate-pulse' : 'bg-red-400'}`}></span>
                            HA {status.ha_enabled ? 'Enabled' : 'Disabled'}
                        </span>
                        <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium ${status.auto_fence ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-gray-500/20 text-gray-400 border border-gray-500/30'}`}>
            ...
            <div className="flex gap-1 mb-6 border-b border-gray-700/50">
                {tabs.map(t => (
                    <button
                        key={t.key}
                        onClick={() => { setTab(t.key); setSelectedEvac(null) }}
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div><span className="text-gray-500">Heartbeat Timeout:</span> <span className="text-white ml-2">{status.heartbeat_timeout}</span></div>
                            <div><span className="text-gray-500">Monitor Interval:</span> <span className="text-white ml-2">{status.monitor_interval}</span></div>
                        </div>
                    </div>
                            ...
                            <div className="space-y-2">
                                {status.active_fencing.map(f => (
                                    <div key={f.host_id} className="flex items-center justify-between py-2 px-3 bg-red-500/5 rounded-lg">
                                        ...
                                        <button onClick={() => unfenceHost(f.host_id)} className="px-3 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs hover:bg-emerald-500/30 transition">
                                            Unfence
                                        </button>
                                    </div>
                                ))}
                    ...
                    {/* Recent Evacuations */}
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                        <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-3">Recent Evacuations</h3>
                        {(!status.recent_evacuations || status.recent_evacuations.length === 0) ? (
                            ...
                            <div className="space-y-2">
                                {status.recent_evacuations.map(e => (
                                    <div key={e.uuid} className="flex items-center justify-between py-2 px-3 bg-gray-700/20 rounded-lg">
                                        <div className="flex items-center gap-3">
            ...
            {tab === 'policies' && (
                <div className="space-y-4">
                    <div className="flex justify-end">
                        <button onClick={() => setShowCreatePolicy(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-500 transition">
                            + Create Policy
                        </button>
                    </div>
                        ...
                        {policies.map(p => (
                            <div key={p.id} className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {p.enabled ? (
                                        <span className="text-emerald-400 text-xs bg-emerald-500/20 px-2 py-0.5 rounded">ACTIVE</span>
                                    ) : (
                                        <span className="text-gray-400 text-xs bg-gray-500/20 px-2 py-0.5 rounded">DISABLED</span>
                                    )}
                                </div>
                                <div className="grid grid-cols-2 gap-2 text-sm mb-4">
                                    <div><span className="text-gray-500">Priority:</span> <span className="text-white ml-1">{p.priority}</span></div>
                                    <div><span className="text-gray-500">Max Restarts:</span> <span className="text-white ml-1">{p.max_restarts}</span></div>
                                    <div><span className="text-gray-500">Window:</span> <span className="text-white ml-1">{p.restart_window}s</span></div>
                                    <div><span className="text-gray-500">Delay:</span> <span className="text-white ml-1">{p.restart_delay}s</span></div>
                                </div>
                    ...
                    {showCreatePolicy && <CreatePolicyModal onSubmit={createPolicy} onClose={() => setShowCreatePolicy(false)} />}
                </div>
            )}
                ...
                <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                    {instances.length === 0 ? (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {instances.map(inst => (
                                    <tr key={inst.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition">
                                        <td className="px-4 py-3 text-white font-medium">{inst.instance_name || `Instance #${inst.instance_id}`}</td>
                                        <td className="px-4 py-3"><span className={statusBadge(inst.instance_status)}>{inst.instance_status}</span></td>
                                        <td className="px-4 py-3">
                                            <span className={`w-2 h-2 rounded-full inline-block ${inst.ha_enabled ? 'bg-emerald-400' : 'bg-gray-500'}`}></span>
                                            <span className={`ml-2 ${inst.ha_enabled ? 'text-emerald-400' : 'text-gray-500'}`}>
                                                {inst.ha_enabled ? 'Protected' : 'Unprotected'}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-gray-300">{inst.priority}</td>
                                        <td className="px-4 py-3 text-gray-300">{inst.restart_count} / {inst.max_restarts}</td>
                                        <td className="px-4 py-3 text-gray-400 text-xs">{formatTime(inst.last_restart)}</td>
                                        <td className="px-4 py-3">
                                            <button
                ...
                <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                    {evacuations.length === 0 ? (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {evacuations.map(e => (
                                    <tr key={e.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition cursor-pointer" onClick={() => viewEvacDetails(e)}>
                                        <td className="px-4 py-3 text-white font-medium">{e.source_host_name}</td>
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-0.5 rounded text-xs ${e.trigger === 'heartbeat_timeout' ? 'bg-red-500/20 text-red-400' : e.trigger === 'maintenance' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                                {e.trigger}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-gray-400 text-xs">
                                            {e.completed_at ? `${Math.round((new Date(e.completed_at).getTime() - new Date(e.started_at).getTime()) / 1000)}s` : '—'}
                                        </td>
            ...
            {tab === 'evacuations' && selectedEvac && (
                <div className="space-y-4">
                    <button onClick={() => setSelectedEvac(null)} className="text-sm text-gray-400 hover:text-white transition">← Back to Evacuations</button>
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                <h3 className="text-white font-semibold text-lg">{selectedEvac.event.source_host_name}</h3>
                                <p className="text-gray-400 text-sm">{selectedEvac.event.trigger} • {formatTime(selectedEvac.event.started_at)}</p>
                            </div>
                            <span className={statusBadge(selectedEvac.event.status)}>{selectedEvac.event.status}</span>
                        </div>
                        <div className="grid grid-cols-4 gap-4 text-sm mb-4">
                            <div className="bg-gray-700/20 rounded-lg p-3 text-center">
                                <div className="text-2xl font-bold text-white">{selectedEvac.event.total_instances}</div>
                                <div className="text-gray-400 text-xs">Total</div>
                            </div>
                            <div className="bg-emerald-500/10 rounded-lg p-3 text-center">
                                <div className="text-2xl font-bold text-emerald-400">{selectedEvac.event.evacuated}</div>
                            ...
                            <tbody>
                                {selectedEvac.instances.map(inst => (
                                    <tr key={inst.id} className="border-t border-gray-700/30">
                                        <td className="px-4 py-3 text-white">{inst.instance_name || `#${inst.instance_id}`}</td>
                                        <td className="px-4 py-3"><span className={statusBadge(inst.status)}>{inst.status}</span></td>
                ...
                <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                    {fencingEvents.length === 0 ? (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {fencingEvents.map(f => (
                                    <tr key={f.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition">
                                        ...
                                        <td className="px-4 py-3 text-gray-400 text-xs max-w-[200px] truncate">{f.reason}</td>
...
function CreatePolicyModal({ onSubmit, onClose }: { onSubmit: (d: Partial<HAPolicy>) => void; onClose: () => void }) {
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 w-[480px]" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" placeholder="e.g. database-tier" />
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                        <div>
                            <label className="block text-sm text-gray-400 mb-1">Priority</label>
                            <input type="number" value={priority} onChange={e => setPriority(parseInt(e.target.value) || 0)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" />
                        </div>
                        <div>
                            <label className="block text-sm text-gray-400 mb-1">Max Restarts</label>
                            <input type="number" value={maxRestarts} onChange={e => setMaxRestarts(parseInt(e.target.value) || 3)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" />
                        </div>
                        <div>
                            <label className="block text-sm text-gray-400 mb-1">Window (s)</label>
                            <input type="number" value={restartWindow} onChange={e => setRestartWindow(parseInt(e.target.value) || 3600)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" />
                        </div>
                    </div>
                </div>
                <div className="flex justify-end gap-3 mt-6">
                    <button onClick={onClose} className="px-4 py-2 text-gray-400 hover:text-white text-sm transition">Cancel</button>
                    <button onClick={() => onSubmit({ name, priority, max_restarts: maxRestarts, restart_window: restartWindow, enabled: true })} disabled={!name} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-500 transition disabled:opacity-50">Create</button>
                </div>
            </div>
        </div>
    )
