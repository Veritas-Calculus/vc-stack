...
export function DNSManagement() {
    ...
    const filteredZones = zones.filter(z =>
        !searchFilter || z.name.toLowerCase().includes(searchFilter.toLowerCase())
    )
        ...
        <div>
            <div className="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">DNS as a Service — Manage zones and record sets</p>
                </div>
                <button
                                        ...
                                        <tbody className="divide-y divide-oxide-800/50">
                                            {filteredZones.map((z) => (
                                                <tr
                                                    ...
                                                    onClick={() => { setSelectedZone(z); setTab('records') }}
                                                >
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span className="px-2 py-0.5 rounded text-xs bg-oxide-700 text-gray-300">{z.type}</span>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <span className={`px-2 py-0.5 rounded text-xs ${statusColor(z.status)}`}>{z.status}</span>
                                                    </td>
                                                    <td className="px-4 py-3 text-center text-gray-300">{z.recordset_count}</td>
                                    ...
                                    <div className="mb-4 p-4 rounded-xl border border-oxide-800 bg-oxide-900/50 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-400 mt-1">
                                                Serial: <span className="font-mono">{selectedZone.serial}</span> · TTL: {selectedZone.ttl}s · Email: {selectedZone.email || '—'}
                                            </p>
                                        </div>
                                                ...
                                                <tbody className="divide-y divide-oxide-800/50">
                                                    {records.map((r) => (
                                                        <tr key={r.id} className="hover:bg-oxide-800/30 transition-colors">
                                                            ...
                                                            <td className="px-4 py-3">
                                                                <span className={`px-2 py-0.5 rounded text-xs font-medium ${r.type === 'A' || r.type === 'AAAA' ? 'bg-blue-500/15 text-blue-400' :
                                                                        r.type === 'CNAME' ? 'bg-purple-500/15 text-purple-400' :
                                                                            r.type === 'MX' ? 'bg-orange-500/15 text-orange-400' :
                                                                                r.type === 'TXT' || r.type === 'SPF' ? 'bg-yellow-500/15 text-yellow-400' :
                                                                                    r.type === 'SOA' || r.type === 'NS' ? 'bg-gray-500/15 text-gray-400' :
                                                                                        'bg-teal-500/15 text-teal-400'
                                                                    }`}>{r.type}</span>
                                                            </td>
                                                            <td className="px-4 py-3 font-mono text-xs text-gray-300 max-w-xs truncate">
                                                                {r.priority > 0 && <span className="text-gray-500 mr-1">{r.priority}</span>}
                                                                {r.records}
                                                            </td>
                                                            <td className="px-4 py-3 text-gray-400">{r.ttl ?? selectedZone.ttl}s</td>
                                                            <td className="px-4 py-3">
                                                                <span className={`px-2 py-0.5 rounded text-xs ${statusColor(r.status)}`}>{r.status}</span>
                                                            </td>
                                                            <td className="px-4 py-3 text-right">
                                                                {!(r.name === selectedZone.name && (r.type === 'SOA' || r.type === 'NS')) && (
                                                                    <button
            ...
            {showCreateRecord && selectedZone && (
                <Modal title={`Add Record — ${selectedZone.name}`} onClose={() => setShowCreateRecord(false)}>
                    <div className="space-y-4">
                        ...
                        <Field label="Data" required>
                            <input
                                type="text"
                                placeholder={recordType === 'A' ? '192.168.1.1' : recordType === 'CNAME' ? 'target.example.com.' : 'value'}
                                value={recordData}
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-oxide-900 border border-oxide-700 rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6" onClick={(e) => e.stopPropagation()}>
