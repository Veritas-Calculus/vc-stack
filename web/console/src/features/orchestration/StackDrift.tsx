...
export function StackDrift() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true)
      setVersions(await fetchStackVersions(stackID))
    } catch {
      /* empty */
    } finally {
      setLoading(false)
  ...
  const cols: Column<UIStackVersion>[] = [
    {
      key: 'version',
      header: 'Version',
      render: (r) => <span className="font-mono font-medium">v{r.version}</span>
    },
  ...
  return (
    <div className="space-y-3">
      <PageHeader
        ...
        subtitle="Version history, drift detection, and rollback for orchestration stacks"
      />
      {loading ? (
        <div className="text-center py-12 text-zinc-500">Loading...</div>
      ) : (
        <DataTable columns={cols} data={versions} empty="No stack versions" />
      )}
    </div>
  )
}
