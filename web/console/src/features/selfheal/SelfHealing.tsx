...
export function SelfHealing() {
    ...
    const fetchAll = useCallback(async () => {
        try {
            const [st, ch, po] = await Promise.allSettled([
                api.get('/v1/selfheal/status'), api.get('/v1/selfheal/checks'), api.get('/v1/selfheal/policies')
            ])
    ...
    useEffect(() => {
        if (tab === 'events') api.get('/v1/selfheal/events').then(r => setEvents(r.data.events || [])).catch(() => { })
    }, [tab])

    const runCheck = async (id: string) => {
        try { await api.post(`/v1/selfheal/checks/${id}/run`); fetchAll() } catch { /* */ }
    }

    const simulate = async (type: string) => {
        const check = checks.find(c => c.resource_type === (type === 'vm_crash' ? 'vm' : type === 'disk_full' ? 'volume' : type === 'host_overload' ? 'host' : 'service'))
        try {
    ...
    const badge = (s: string) => {
        const m: Record<string, string> = {
            healthy: 'bg-emerald-500/20 text-emerald-400', warning: 'bg-amber-500/20 text-amber-400',
            critical: 'bg-red-500/20 text-red-400', unknown: 'bg-gray-500/20 text-gray-400',
            success: 'bg-emerald-500/20 text-emerald-400', failed: 'bg-red-500/20 text-red-400',
            triggered: 'bg-blue-500/20 text-blue-400', executing: 'bg-amber-500/20 text-amber-400',
    ...
    const tabs: { key: Tab; label: string }[] = [
        { key: 'dashboard', label: 'Dashboard' }, { key: 'checks', label: 'Health Checks' },
        { key: 'policies', label: 'Healing Policies' }, { key: 'events', label: 'Events' },
    ]
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Automated health monitoring and infrastructure remediation</p>
                </div>
                <div className="flex items-center gap-3">
                    {status && <span className="text-sm text-gray-400">Heal rate: <span className="text-emerald-400 font-bold">{String(status.healing_rate_pct)}%</span></span>}
                    {status && <span className="px-3 py-1 rounded-lg border border-emerald-500/30 text-emerald-400 text-sm">● Active</span>}
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-5 gap-3">
                            {policies.map(p => (
                                <div key={p.id as string} className="bg-gray-700/20 rounded-lg p-3">
                                    ...
                                    <div className="text-xs text-gray-500">{p.resource_type as string} → {p.action as string}</div>
                                    <div className="text-xs text-gray-500 mt-1">P{p.priority as number} • {p.cooldown_min as number}m cooldown</div>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {tab === 'checks' && (
                <div className="space-y-3">
                    {checks.map(c => (
                        <div key={c.id as string} className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`w-3 h-3 rounded-full ${c.status === 'healthy' ? 'bg-emerald-400' : c.status === 'warning' ? 'bg-amber-400' : c.status === 'critical' ? 'bg-red-400' : 'bg-gray-400'}`}></div>
                                    <div>
                                        <div className="text-white font-medium">{c.name as string}</div>
                                        <div className="text-gray-400 text-xs">{c.resource_type as string} • {c.check_type as string} • {c.target as string}</div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <div className="text-right text-xs text-gray-400">
                                        <div>Every {String(c.interval_sec)}s • {String(c.retries)} retries</div>
            ...
            {tab === 'policies' && (
                <div className="space-y-3">
                    {policies.map(p => (
                        <div key={p.id as string} className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <div className="text-white font-medium">{p.name as string}</div>
                                    <div className="text-gray-400 text-xs mt-1">
                                        When <span className={badge(p.trigger_status as string)}>{p.trigger_status as string}</span> on <span className="text-cyan-400">{p.resource_type as string}</span> → <span className="text-amber-400">{p.action as string}</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 text-xs text-gray-400">
                                    ...
                                    {Boolean(p.escalate_action) && (
                                        <div className="text-right">
                                            <div className="text-orange-400">Escalate after {String(p.escalate_after)} fails</div>
                                            <div>→ {String(p.escalate_action)}</div>
                                        </div>
                                    )}
                                    <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400">P{p.priority as number}</span>
                                </div>
                            </div>
                        </div>
                    ))}
