...
export function Isos() {
  ...
  const isos = useMemo(() => rows.filter((r) => r.disk_format === 'iso' && (!search || r.name.includes(search))), [rows, search])
    ...
    <div className="space-y-3">
      ...
      >
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">上传 ISO 文件用于手动安装系统，保存到后端的 VC_IMAGE_DIR。</p>
        </div>
      </Modal>
      <Modal
        ...
        footer={
          <>
            ...
            <RegisterFromUrl kind="iso" onDone={async () => { setRows(await fetchImages()); setOpenRegister(false) }} />
          </>
        }
      >
...
export function K8sIsos() {
  ...
  const [search, setSearch] = useState('')
  useEffect(() => { (async () => setRows(await fetchImages()))() }, [])
  const data = rows.filter((r) => r.disk_format === 'iso' && /k8s|kubernetes/i.test(r.name) && (!search || r.name.includes(search)))
  const cols: Column<UIImage>[] = [
    { key: 'name', header: 'Name' },
    { key: 'status', header: 'State', render: (r) => <Badge variant={r.status === 'active' || r.status === 'available' ? 'success' : 'info'}>{r.status}</Badge> },
    { key: 'disk_format', header: 'OS Type', render: () => <span className="uppercase">ISO</span> },
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <div className="space-y-3">
          <input type="file" accept=".iso" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
          <p className="text-xs text-muted-foreground">上传 Kubernetes 集群节点镜像 ISO，保存到后端的 VC_IMAGE_DIR。</p>
        </div>
      </Modal>
      <Modal
