...
export function ServiceCatalog() {
    ...
    const fetchAll = useCallback(async () => {
        try {
            const [st, ct, it, ft] = await Promise.allSettled([
                api.get('/v1/catalog/status'), api.get('/v1/catalog/categories'),
                api.get('/v1/catalog/items'), api.get('/v1/catalog/featured')
            ])
    ...
    useEffect(() => {
        if (tab === 'requests') {
            api.get('/v1/catalog/requests').then(r => setRequests(r.data.requests || [])).catch(() => { })
        }
    }, [tab])
    ...
    const parseSpecs = (s: string) => { try { return JSON.parse(s) } catch { return {} } }
    ...
    const tabs: { key: Tab; label: string }[] = [
        { key: 'browse', label: 'Browse Catalog' }, { key: 'featured', label: 'Featured' }, { key: 'requests', label: 'My Requests' }
    ]
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Browse and provision infrastructure services on demand</p>
                </div>
                {status && <div className="flex gap-4 text-xs text-gray-400">
            ...
            <div className="flex gap-1 mb-6 bg-gray-800/40 p-1 rounded-lg w-fit">
                {tabs.map(t => (
                    <button key={t.key} onClick={() => { setTab(t.key); setSelectedItem(null) }} className={`px-4 py-2 rounded-md text-sm font-medium transition ${tab === t.key ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-200'}`}>{t.label}</button>
                ))}
            </div>
            ...
            {tab === 'browse' && !selectedItem && (
                <div className="flex gap-6">
                    ...
                    <div className="w-56 shrink-0 space-y-1">
                        <button onClick={() => setSelectedCat(null)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${!selectedCat ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:text-white hover:bg-gray-700/30'}`}>All ({items.length})</button>
                        {categories.map(cat => (
                            <button key={cat.id} onClick={() => setSelectedCat(cat.id)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${selectedCat === cat.id ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:text-white hover:bg-gray-700/30'}`}>
                                <span className="mr-2">{cat.icon}</span>{cat.name} ({cat.item_count})
                            </button>
                        ))}
                    </div>
                    {/* Items grid */}
                    <div className="flex-1 grid grid-cols-2 gap-4">
                        {filteredItems.map(item => (
                            <div key={item.id} onClick={() => setSelectedItem(item)} className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5 cursor-pointer hover:border-blue-500/40 transition group">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="text-lg font-bold text-emerald-400">{formatPrice(item.price_amount, item.price_unit)}</div>
            ...
            {tab === 'browse' && selectedItem && (
                <div className="space-y-4">
                    <button onClick={() => setSelectedItem(null)} className="text-gray-400 hover:text-white text-sm">← Back to Catalog</button>
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-6">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <div className="bg-gray-700/20 rounded-lg p-4">
                                ...
                                <div className="text-xl font-bold text-emerald-400">{formatPrice(selectedItem.price_amount, selectedItem.price_unit)}</div>
                            </div>
                            <div className="bg-gray-700/20 rounded-lg p-4">
                            ...
                            <div className="bg-gray-700/20 rounded-lg p-4">
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {parseTags(selectedItem.tags).map((tag: string) => (
                                        <span key={tag} className="px-2 py-0.5 bg-gray-600/40 text-gray-300 text-xs rounded">{tag}</span>
                                    ))}
                                </div>
                                <div className="mt-3 text-xs text-gray-500">{selectedItem.deployments} deployments</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {tab === 'featured' && (
                <div className="grid grid-cols-3 gap-4">
                    {featured.map(item => (
                        <div key={item.id} onClick={() => { setSelectedItem(item); setTab('browse') }} className="bg-gradient-to-br from-gray-800/80 to-gray-700/40 border border-amber-500/20 rounded-xl p-5 cursor-pointer hover:border-amber-400/50 transition">
                            <div className="w-12 h-12 rounded-lg bg-amber-500/10 flex items-center justify-center text-2xl mb-3">{item.icon}</div>
                            <h3 className="text-white font-bold text-lg mb-1">{item.display_name}</h3>
                            <p className="text-gray-400 text-sm mb-4 line-clamp-2">{item.description}</p>
                            <div className="flex items-center justify-between">
                                <span className="text-emerald-400 font-bold">{formatPrice(item.price_amount, item.price_unit)}</span>
                        ...
                        <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                            <table className="w-full text-sm">
                                <thead className="bg-gray-700/30"><tr className="text-left text-gray-400 text-xs uppercase">
                                    <th className="px-4 py-3">Service</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Requested</th>
                                </tr></thead>
                                <tbody>{requests.map((r: unknown) => {
                                    const req = r as Record<string, unknown>
                                    const st = req.status as string
                                    const stColor = st === 'completed' ? 'text-emerald-400 bg-emerald-500/20' : st === 'pending' ? 'text-amber-400 bg-amber-500/20' : st === 'rejected' ? 'text-red-400 bg-red-500/20' : 'text-gray-400 bg-gray-500/20'
                                    return (
                                        <tr key={req.id as string} className="border-t border-gray-700/30">
                                            ...
                                            <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs ${stColor}`}>{st}</span></td>
                                            <td className="px-4 py-3 text-gray-400 text-xs">{req.created_at ? new Date(req.created_at as string).toLocaleString() : '—'}</td>
                                        </tr>
                                    )
                                })}</tbody>
                            </table>
