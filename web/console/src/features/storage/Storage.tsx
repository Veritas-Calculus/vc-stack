...
import { createVolume, createVolumeSnapshot, deleteVolume, resizeVolume, fetchAudit, fetchVolumeSnapshots, fetchVolumes, type UIVolume, type UIVolumeSnapshot, type UIAudit } from '@/lib/api'
...
function Volumes() {
      ...
      render: (row) => (
        <div className="flex gap-2">
          ...
          <button 
            ...
            title={row.status === 'in-use' ? 'Volume is in use by an instance; detach or delete the instance first' : 'Delete volume'}
            onClick={async () => {
              ...
              if (confirm(`Delete volume "${row.name}"?`)) {
                try {
                  ...
                  if (axios.isAxiosError(err) && err.response?.status === 409) {
                    alert('Volume is in use by an instance; please detach or delete the instance first.')
                  } else {
  ...
  return (
    <div className="space-y-3">
      <PageHeader title="Volumes" subtitle="Project volumes" actions={
        <div className="flex gap-2">
          <button className="btn-secondary" onClick={load} disabled={loading}>
            {loading ? 'Loading...' : 'Refresh'}
          </button>
          <button className="btn-primary" onClick={() => setOpen(true)}>Create Volume</button>
        </div>
      } />
      ...
      {/* Create Volume Modal */}
      <Modal title="Create Volume" open={open} onClose={() => setOpen(false)} footer={<>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input className="input w-full" type="number" min="1" value={size} onChange={(e) => setSize(e.target.value ? Number(e.target.value) : '')} />
          </div>
        </div>
      </Modal>
      
      {/* Resize Volume Modal */}
      <Modal title="Resize Volume" open={resizeOpen} onClose={() => setResizeOpen(false)} footer={<>
        <button className="btn-secondary" onClick={() => setResizeOpen(false)}>Cancel</button>
        <button 
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input 
              ...
              value={resizeNewSize} 
              onChange={(e) => setResizeNewSize(e.target.value ? Number(e.target.value) : resizeCurrentSize)} 
            />
...
function Snapshots() {
  ...
  useEffect(() => { (async () => setRows(await fetchVolumeSnapshots(projectId)))() }, [projectId])
  const cols: Column<typeof rows[number]>[] = [
  ...
  return (
    <div className="space-y-3">
      <PageHeader title="Snapshots" subtitle="Volume snapshots" actions={
        <div className="flex gap-2">
          <button className="btn-secondary" onClick={async () => { setRows(await fetchVolumeSnapshots(projectId)) }}>Refresh</button>
          <button className="btn-primary" onClick={() => setOpen(true)}>Create Snapshot</button>
        </div>
      } />
      <DataTable columns={cols} data={rows} empty="No snapshots" />
      <Modal title="Create Volume Snapshot" open={open} onClose={() => setOpen(false)} footer={<>
        <button className="btn-secondary" onClick={() => setOpen(false)}>Cancel</button>
        <button className="btn-primary" disabled={busy} onClick={async () => { if (projectId && source) { setBusy(true); await createVolumeSnapshot(projectId, { name: name || `snap-${Date.now()}`, volume_id: Number(source) }); setName(''); setSource(''); setOpen(false); setRows(await fetchVolumeSnapshots(projectId)); setBusy(false) } }}>Create</button>
      </>}>
        <div className="space-y-3">
          <div>
            <label className="label">Name</label>
...
function Backups() {
  ...
  useEffect(() => { (async () => setRows(await fetchAudit(projectId, { resource: 'snapshot' })))() }, [projectId])
  ...
  return (
    <div className="space-y-3">
      <PageHeader title="Backups" subtitle="Recent backup operations (audit)" actions={<button className="btn-secondary" onClick={async () => setRows(await fetchAudit(projectId, { resource: 'snapshot' }))}>Refresh</button>} />
