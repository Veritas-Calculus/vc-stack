...
import { fetchVolumeById, fetchVolumeSnapshotsByVolume, cloneVolume, detachVolumeFromVM } from '@/lib/api'
...
export default function VolumeDetail() {
    ...
    const attachCols: Column<Attachment>[] = [
        ...
        { key: 'created_at', header: 'Attached At', render: (r) => new Date(r.created_at).toLocaleString() },
    ]

    const snapCols: Column<VolumeSnapshot>[] = [
        { key: 'id', header: 'ID' },
        { key: 'name', header: 'Name' },
        { key: 'status', header: 'Status', render: (r) => <Badge variant={statusColors[r.status]}>{r.status}</Badge> },
        { key: 'size_bytes', header: 'Size', render: (r) => r.size_bytes > 0 ? `${Math.round(r.size_bytes / 1073741824)} GB` : '-' },
    ]
        ...
        <div className="p-6 space-y-5">
            <PageHeader
                ...
                actions={
                    <div className="flex gap-2">
                        <button className="btn-secondary text-sm" onClick={() => navigate(-1)}>Back</button>
                        <button className="btn-secondary text-sm" onClick={handleClone}>Clone</button>
            ...
            <div className="flex gap-3 items-center flex-wrap">
                ...
                {volume.disk_offering && (
                    <Badge>{volume.disk_offering.name} ({volume.disk_offering.storage_type})</Badge>
                )}
            </div>
            ...
            {tab === 'info' && (
                <div className="bg-[var(--card-bg,#1a1a2e)] border border-[var(--border-primary,#2a2a4a)] rounded-xl p-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        {volume.source_snapshot_id && <InfoRow label="From Snapshot" value={`#${volume.source_snapshot_id}`} />}
                        {volume.source_image_id && <InfoRow label="From Image" value={`#${volume.source_image_id}`} />}
                        {volume.source_volume_id && <InfoRow label="Cloned From" value={`#${volume.source_volume_id}`} />}
                        {volume.disk_offering && (
                            <>
                                <InfoRow label="Storage Class" value={volume.disk_offering.name} />
                                {volume.disk_offering.max_iops > 0 && <InfoRow label="Max IOPS" value={String(volume.disk_offering.max_iops)} />}
                                {volume.disk_offering.throughput > 0 && <InfoRow label="Throughput" value={`${volume.disk_offering.throughput} MB/s`} />}
                            </>
                        )}
                    </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-[var(--border-primary,#2a2a4a)]">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(volume.metadata).map(([k, v]) => (
                                    <span key={k} className="text-xs bg-gray-800 border border-gray-700 rounded px-2 py-0.5">
                                        {k}={v}
                                    </span>
                                ))}
                            </div>
                    ...
                    <DataTable
                        columns={[
                            ...attachCols,
                            {
                                ...
                                sortable: false,
                                render: (r) => (
                                    <button className="text-red-400 hover:text-red-300 text-xs" onClick={() => handleDetach(r.instance_id)}>
                                        Detach
                                    </button>
                                ),
                            },
