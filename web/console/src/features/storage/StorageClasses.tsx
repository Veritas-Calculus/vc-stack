...
export default function StorageClasses() {
    ...
    const cols: Column<DiskOffering>[] = [
        ...
        {
            key: 'storage_type', header: 'Type', render: (r) => (
                <Badge>{storageTypeLabel[r.storage_type] || r.storage_type}</Badge>
            )
        },
        {
            key: 'disk_size_gb', header: 'Size', render: (r) =>
                r.is_custom ? <span className="text-gray-400 text-xs">Custom</span> : `${r.disk_size_gb} GB`
        },
        {
            key: 'max_iops', header: 'IOPS', render: (r) =>
                r.max_iops > 0
                    ? <span>{r.min_iops > 0 ? `${r.min_iops}–` : ''}{r.max_iops}</span>
                    : <span className="text-gray-500">-</span>
        },
        {
            key: 'throughput', header: 'Throughput', render: (r) =>
                r.throughput > 0 ? `${r.throughput} MB/s` : '-'
        },
        {
            ...
            >
                <div className="space-y-3">
                    <div>
                        ...
                        <input className="input w-full" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. gold-ssd" />
                    </div>
                    <div>
                        <label className="label">Display Text</label>
                        <input className="input w-full" value={displayText} onChange={(e) => setDisplayText(e.target.value)} placeholder="Description" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="label">Storage Type</label>
                            <select className="input w-full" value={storageType} onChange={(e) => setStorageType(e.target.value)}>
                        ...
                        <div>
                            ...
                            <input className="input w-full" type="number" min="0" value={diskSize}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <label htmlFor="is_custom" className="text-sm text-gray-300">Allow custom size</label>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                        <div>
                            <label className="label">Min IOPS</label>
                            <input className="input w-full" type="number" min="0" value={minIops}
                                onChange={(e) => setMinIops(e.target.value ? Number(e.target.value) : '')} />
                        </div>
                        <div>
                            <label className="label">Max IOPS</label>
                            <input className="input w-full" type="number" min="0" value={maxIops}
                                onChange={(e) => setMaxIops(e.target.value ? Number(e.target.value) : '')} />
                        </div>
                        <div>
                            <label className="label">Throughput (MB/s)</label>
                            <input className="input w-full" type="number" min="0" value={throughput}
