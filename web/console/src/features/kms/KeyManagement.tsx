...
export function KeyManagement() {
    ...
    const fetchData = useCallback(async () => {
        ...
        } catch (err) { console.error('KMS fetch error:', err) }
        setLoading(false)
    }, [])
    ...
    const createSecret = async (data: Record<string, unknown>) => {
        try {
            ...
            fetchData()
        } catch (err) { console.error('Create secret error:', err) }
    }
    ...
    const deleteKey = async (id: number) => {
        if (!confirm('Permanently destroy this encryption key? Data encrypted with this key will become unrecoverable.')) return
        try {
    ...
    const rotateKey = async (id: number) => {
        if (!confirm('Rotate this key? A new key version will be created and the old version deactivated.')) return
        try {
            await api.post(`/v1/kms/keys/${id}/rotate`)
            fetchData()
        } catch (err) { console.error('Rotate key error:', err) }
    }
    ...
    const doEncrypt = async () => {
        ...
        } catch (err) { console.error('Encrypt error:', err); setCryptoResult('Error: encryption failed') }
    }
    ...
    const doDecrypt = async () => {
        ...
        } catch (err) { console.error('Decrypt error:', err); setCryptoResult('Error: decryption failed') }
    }
    ...
    const doGenerateDEK = async () => {
        ...
        try {
            ...
            setCryptoResult(JSON.stringify({
                plaintext_dek: res.data.plaintext,
                wrapped_dek: res.data.ciphertext,
                bit_length: res.data.bit_length,
            }, null, 2))
        } catch (err) { console.error('Generate DEK error:', err); setCryptoResult('Error: DEK generation failed') }
    }
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Secret storage, encryption key lifecycle, and data encryption</p>
                </div>
                {status && (
                    <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium ${status.status === 'operational' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                        <span className={`w-2 h-2 rounded-full mr-2 ${status.status === 'operational' ? 'bg-emerald-400 animate-pulse' : 'bg-red-400'}`}></span>
            ...
            <div className="flex gap-1 mb-6 border-b border-gray-700/50">
                {tabs.map(t => (
                    ...
                    >
                        {t.label}
                        {t.count !== null && <span className="ml-2 px-1.5 py-0.5 bg-gray-700/60 rounded text-xs">{t.count}</span>}
                    </button>
                ))}
            </div>
            ...
            {tab === 'overview' && status && (
                <div className="space-y-6">
                    ...
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                        <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-3">Configuration</h3>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div><span className="text-gray-500">Algorithm:</span> <span className="text-white ml-2">{status.algorithm}</span></div>
                            <div><span className="text-gray-500">Master Key:</span> <span className={`ml-2 ${status.master_key_loaded ? 'text-emerald-400' : 'text-red-400'}`}>{status.master_key_loaded ? 'Loaded' : 'Not loaded'}</span></div>
                        </div>
                    </div>

                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
                        <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-3">Envelope Encryption</h3>
                        <div className="text-sm text-gray-400 space-y-2">
                            <p>KMS uses <span className="text-white font-medium">envelope encryption</span> to protect data:</p>
                            <div className="flex items-center gap-2 py-2">
                                <span className="px-2 py-1 bg-purple-500/20 text-purple-400 rounded text-xs">Master Key (KEK)</span>
                                <span className="text-gray-500">→ encrypts →</span>
                                <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-xs">Data Encryption Key (DEK)</span>
                                <span className="text-gray-500">→ encrypts →</span>
                                <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs">Your Data</span>
                            </div>
                            <p className="text-xs text-gray-500">Generate a DEK with <code className="bg-gray-700/60 px-1 rounded">/api/v1/kms/generate-dek</code>, encrypt your data locally, then store only the wrapped DEK.</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {tab === 'secrets' && (
                <div className="space-y-4">
                    <div className="flex justify-end">
                        <button onClick={() => setShowCreateSecret(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-500 transition">
                            + Store Secret
                        </button>
                    </div>
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                        {secrets.length === 0 ? (
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {secrets.map(sec => (
                                        <tr key={sec.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition">
                                            <td className="px-4 py-3">
                                                <div className="text-white font-medium">{sec.name}</div>
                                                <div className="text-gray-500 text-xs font-mono">{sec.uuid.slice(0, 8)}...</div>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className="px-2 py-0.5 bg-purple-500/20 text-purple-400 rounded text-xs">{sec.secret_type}</span>
                                            </td>
                                            <td className="px-4 py-3 text-gray-300">
                                                {sec.algorithm ? `${sec.algorithm.toUpperCase()}-${sec.bit_length}` : '—'}
                                            </td>
                                            <td className="px-4 py-3"><span className={statusBadge(sec.status)}>{sec.status}</span></td>
                                            <td className="px-4 py-3 text-gray-400 text-xs">{formatTime(sec.created_at)}</td>
                                            <td className="px-4 py-3">
                                                <button onClick={() => deleteSecret(sec.id)} className="text-red-400 text-xs hover:text-red-300 transition">Destroy</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                    ...
                    {showCreateSecret && <CreateSecretModal onSubmit={createSecret} onClose={() => setShowCreateSecret(false)} />}
                </div>
            )}
            ...
            {tab === 'keys' && (
                <div className="space-y-4">
                    <div className="flex justify-end">
                        <button onClick={() => setShowCreateKey(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-500 transition">
                            + Create Key
                        </button>
                    </div>
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl overflow-hidden">
                        {keys.length === 0 ? (
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {keys.map(k => (
                                        <tr key={k.id} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition">
                                            <td className="px-4 py-3">
                                                <div className="text-white font-medium">{k.name}</div>
                                                <div className="text-gray-500 text-xs font-mono">{k.uuid.slice(0, 8)}...</div>
                                            </td>
                                            <td className="px-4 py-3 text-gray-300">{k.algorithm.toUpperCase()}-{k.bit_length}-{k.mode.toUpperCase()}</td>
                                            <td className="px-4 py-3"><span className={statusBadge(k.status)}>{k.status}</span></td>
                                            <td className="px-4 py-3 text-gray-300">{k.usage_count.toLocaleString()}</td>
                                            <td className="px-4 py-3 text-gray-400 text-xs">{k.rotated_at ? formatTime(k.rotated_at) : '—'}</td>
                                            <td className="px-4 py-3 text-gray-400 text-xs">{formatTime(k.created_at)}</td>
                                            <td className="px-4 py-3">
                                                <div className="flex gap-2">
                                                    {k.status === 'active' && (
                                                        <button onClick={() => rotateKey(k.id)} className="text-amber-400 text-xs hover:text-amber-300 transition">Rotate</button>
                                                    )}
                                                    <button onClick={() => deleteKey(k.id)} className="text-red-400 text-xs hover:text-red-300 transition">Destroy</button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                    ...
                    {showCreateKey && <CreateKeyModal onSubmit={createKey} onClose={() => setShowCreateKey(false)} />}
                </div>
            )}
                    ...
                    <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-6">
                        ...
                        <div className="mb-4">
                            ...
                            >
                                ...
                                {keys.filter(k => k.status === 'active' || (cryptoMode === 'decrypt' && k.status === 'deactivated')).map(k => (
                                    <option key={k.uuid} value={k.uuid}>{k.name} ({k.algorithm.toUpperCase()}-{k.bit_length})</option>
                                ))}
                            </select>
                        </div>
                        ...
                        <button
                            ...
                            disabled={!selectedKeyId || (cryptoMode === 'encrypt' && !plaintext) || (cryptoMode === 'decrypt' && !ciphertext)}
                            className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-500 transition disabled:opacity-50"
                        >
                            {cryptoMode === 'encrypt' ? 'Encrypt' : cryptoMode === 'decrypt' ? 'Decrypt' : 'Generate DEK'}
                        </button>
...
function CreateSecretModal({ onSubmit, onClose }: { onSubmit: (d: Record<string, unknown>) => void; onClose: () => void }) {
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 w-[520px]" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" placeholder="e.g. db-password" />
                    </div>
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">Type</label>
                        <select value={secretType} onChange={e => setSecretType(e.target.value)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none">
                    ...
                    {secretType !== 'symmetric' && (
                        <div>
                            ...
                            <textarea value={payload} onChange={e => setPayload(e.target.value)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm h-24 font-mono focus:border-blue-500 outline-none" placeholder="Paste secret value..." />
                            <p className="text-gray-500 text-xs mt-1">The value will be base64-encoded and encrypted before storage.</p>
                        </div>
                    )}
                    {secretType === 'symmetric' && (
                        <p className="text-gray-500 text-sm bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">A 256-bit symmetric key will be auto-generated.</p>
                    )}
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">Description (optional)</label>
                        <input value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" />
                    </div>
                </div>
...
function CreateKeyModal({ onSubmit, onClose }: { onSubmit: (d: Record<string, unknown>) => void; onClose: () => void }) {
    const [name, setName] = useState('')
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 w-[480px]" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">Name</label>
                        <input value={name} onChange={e => setName(e.target.value)} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none" placeholder="e.g. volume-encryption-key" />
                    </div>
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">Key Size</label>
                        <select value={bitLength} onChange={e => setBitLength(parseInt(e.target.value))} className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:border-blue-500 outline-none">
                ...
                <div className="flex justify-end gap-3 mt-6">
                    <button onClick={onClose} className="px-4 py-2 text-gray-400 hover:text-white text-sm transition">Cancel</button>
                    <button
                        onClick={() => onSubmit({ name, algorithm: 'aes', bit_length: bitLength, mode: 'gcm', description })}
                        disabled={!name}
                        className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-500 transition disabled:opacity-50"
                    >Create Key</button>
                </div>
