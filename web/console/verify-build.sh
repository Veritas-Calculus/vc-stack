...
if grep -r "https://" "$DIST_DIR"/*.html 2>/dev/null | grep -v "https://api" | grep -v "https://rgw" | grep -v "https://example"; then
    echo "[WARN]️  警告: 发现外部 HTTPS 引用"
else
    echo "[OK] HTML 文件无外部引用"
fi
...
if [ -n "$CDN_FILES" ]; then
    echo "[ERROR] 错误: 发现 CDN 引用"
    echo "$CDN_FILES"
    exit 1
else
    echo "[OK] JS 文件无 CDN 引用"
fi
...
if [ -n "$FONT_FILES" ]; then
    echo "[ERROR] 错误: 发现外部字体引用"
    echo "$FONT_FILES"
    exit 1
else
    echo "[OK] CSS 文件无外部字体引用"
fi
